...
import { Sun, Moon, Trash2, Pencil, LayoutDashboard, LogOut, Plus } from 'lucide-react';
                ...
                {boards.map(board => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition ml-1 p-1 rounded-md flex-shrink-0"
                      title="Eliminar tablero"
                    >
                      <Trash2 size={14} />
                    </button>
...
function TaskCard({
  ...
  className={`text-sm font-medium cursor-pointer flex-1 transition-all ${
    task.completed
      ? 'line-through text-green-400 dark:text-green-400'
      : 'text-gray-900 dark:text-white'
  }`}
  onClick={() => onToggle(task.id, columnId, task.completed)}
